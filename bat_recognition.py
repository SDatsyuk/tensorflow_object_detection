...
try:
    ssh_session = sshClient('172.17.110.99', '22', 'pi', 'raspberry')
...
print('sender')
try:
    res = sshCommand(ssh_session, "sudo python3 send_photo.py").decode().strip()
    print(res)
...
image = Image.open(os.path.join(r'\\172.17.110.165\share\temp\IT_photos', image_path))
...
image_np = image_np[750:-200, 450:-950]
cv2.imwrite('cropped.jpg', image_np)
# image_np = image
...
plt.imshow(image_np)
plt.savefig(os.path.join('test_images', 'results', 'bat', image_path))
plt.show()
...
for i in range(100):
    # print(output_dict['detection_scores'])
    if output_dict['detection_scores'][i] < .01:
        break
...
print(boxes)
# print("XML main")
# annotation = create_xml_main(folder_text=args['input_dir'], filename_text=image_path, image_shape=image.size)
...
# annotation_str = ET.tostring(annotation)
